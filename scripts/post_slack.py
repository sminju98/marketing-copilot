#!/usr/bin/env python3
"""슬랙 Incoming Webhook 으로 브리핑·마케팅 리포트를 보낸다.

Incoming Webhook 은 OAuth 없이 URL 하나로 동작하므로, 예약(cron) 실행에서도 안정적이다.

사용:
  python3 scripts/post_slack.py --to private --file last_brief_morning.md --title "오늘의 마케팅 큐"
  python3 scripts/post_slack.py --to team --text "..." --title "주간 마케팅 리포트"
  echo "본문" | python3 scripts/post_slack.py --to private --title "..." --dry-run

--to team    : config.delivery.team.slack_webhook 로 (팀 공유용)
--to private : config.delivery.private.slack_webhook 로 (나만 보기 — 마케팅 담당자 개인)

안전장치(2중 게이트):
  ① --sensitive 를 주면 '공유(team)' 채널로는 절대 보내지 않는다(에러).
  ② 플래그를 안 붙였어도 본문이 '나만 보기'로 보이면(마진·원가·예산 상한·미공개 캠페인·
     미출시 정보·대행사 단가) 공유 채널 전송을 거부한다 — 내용 기반 백스톱.
"""
import argparse
import os
import sys
...
def main():
    ...
    # 내용 기반 백스톱: '나만 보기' 콘텐츠(마진·원가·예산 상한·미공개 캠페인·미출시·대행사 단가)는
    # --sensitive 여부와 무관하게 공유 채널 차단
    if args.to == "team" and looks_private(body):
        raise SystemExit("⛔ 거부: 본문이 '나만 보기(마진·원가·예산 상한·미공개 캠페인·미출시 정보·대행사 단가 등)' 콘텐츠로 보입니다. 공유 채널로 보낼 수 없습니다. --to private 로만 보내세요.")
