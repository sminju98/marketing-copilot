"""공통 유틸: 설정 로드, HTTP 요청, 마케팅 컨텍스트/로컬 최소 마케팅 DB 경로.

외부 라이브러리 없이 파이썬 표준 라이브러리만 사용한다(설치 없이 바로 실행).
스크립트는 저장·전송·계산만 하고, 분석·판단은 클로드가 한다.
"""
import contextlib
import datetime
import json
...
HOME = os.environ.get("MKT_COPILOT_HOME", "").strip() or os.path.expanduser("~/.marketing-copilot")
DATA_DIR = os.path.join(HOME, "data")
BRIEFS_DIR = os.path.join(DATA_DIR, "briefs")
# 마케팅 컨텍스트(단일 진실원): brand/products/audiences/channels/tone/claims/goals
# (+ permissions·imagefactory·_policy) — 브랜드·상품·타깃·말투·쓸 수 있는 주장의 원장.
CONTEXT_DIR = os.path.join(HOME, "context")
# 로컬 최소 마케팅 DB(JSONL, 한 줄 = 한 레코드): signals/opportunities/messages/
# contents/campaigns/assets/performances. GA4·광고계정 커넥터가 없어도 여기서 전 스킬이 동작한다.
LIBRARY_DIR = os.path.join(HOME, "library")
# 오늘의 마케팅 큐(queue/<YYYY-MM-DD>.md) — queue_today.py 가 생성·갱신한다.
QUEUE_DIR = os.path.join(DATA_DIR, "queue")
# 콘텐츠 캘린더(calendar/<YYYY-MM>.md) — 빈칸을 채우려고 퀄리티 바를 낮추지 않는다(편수 목표 금지).
CALENDAR_DIR = os.path.join(DATA_DIR, "calendar")
# 4형제 핸드오프 계약서(8항목 포맷). inbox = 받은 것(Sales·Business), outbox = 보낸 것·회신.
HANDOFF_DIR = os.path.join(HOME, "handoffs")
HANDOFF_INBOX = os.path.join(HANDOFF_DIR, "inbox")
HANDOFF_OUTBOX = os.path.join(HANDOFF_DIR, "outbox")
CONFIG_PATH = os.path.join(HOME, "config.json")
EXAMPLE_CONFIG = os.path.join(ROOT, "config.example.json")


def context_file(name):
    """context/ 안의 특정 마케팅 컨텍스트 파일 경로. 예: context_file('claims.md')."""
...
def user_language(cfg=None):
    ...
    지시문이지 사용자에게 보여줄 글이 아니다. 그래서 기본값이 'auto' 이고,
    명시 값이 있을 때만 그 언어로 고정한다(팀 공용 산출물처럼 언어를 못 박아야 할 때).
    """
    if cfg is None:
...
def load_context(cfg=None, files=None):
    """마케팅 컨텍스트를 읽어 하나의 문자열로 합친다(파일명 헤더 포함). 없으면 빈 문자열.
    files 를 주면 그 파일들만, 아니면 핵심 파일 묶음을 읽는다. permissions/imagefactory/
    _policy 같은 나머지는 호출부가 필요할 때만 files 로 명시해 포함한다."""
    env = os.environ.get("MKT_COPILOT_CONTEXT", "").strip()
    if env:
        return env
    default = ["brand.md", "products.md", "audiences.md", "channels.md",
               "tone.md", "claims.md", "goals.md"]
    parts = []
...
def config_from_options():
    ...
    if name:
        cfg["me"] = {"name": name}
    brand = plugin_option("brand_name")
    if brand:
...
def load_config(path=CONFIG_PATH, soft=False):
    ...
    if not os.path.exists(path):
        ...
        raise SystemExit(
            f"[설정 없음] {path} 가 없습니다.\n"
            "  클로드에게 'AI 마케팅 운영자 설정 시작하자'라고 말하면 대화로 만들어 줍니다.\n"
            "  (또는 플러그인 활성화 시 뜨는 질문에 답하거나, 예약/클라우드면 환경변수 MKT_COPILOT_SLACK_PRIVATE 등으로 대체)"
        )
...
def load_business_context(cfg=None):
    ...
    읽어온다. '마케팅(관심→수요)'은 '회사가 무엇을 왜 파는지' 맥락과 붙어야 정확해진다.
    없으면 빈 문자열."""
    cfg = cfg or {}
    links = cfg.get("links", {})
...
def load_sales_context(cfg=None):
    """연계: Sales Copilot이 깔려 있으면 ICP(이상 고객군·먹힌 각도) 파일을 읽어온다.
    영업에서 검증된 메시지가 광고 카피가 되고, 광고에서 검증된 메시지가 콜드메일
    첫 줄이 된다(메시지 원장 양방향 동기화). 없으면 빈 문자열."""
    cfg = cfg or {}
    links = cfg.get("links", {})
    if links.get("use_sales_context") is False:
        return ""
    base = links.get("sales_copilot_home") or "~/.sales-copilot"
    p = os.path.join(os.path.expanduser(base), "context", "icp.md")
    if not os.path.exists(p):
        return ""
    try:
        with open(p, encoding="utf-8") as f:
...
# 마케팅 민감정보: 마진·원가·예산 상한·미공개 캠페인·미출시 정보·대행사 단가는 팀 채널에 흘리면 안 된다.
PRIVATE_SENTINEL = "MKT-COPILOT:PRIVATE-ONLY"
_PRIVATE_MARKERS = (PRIVATE_SENTINEL, "마진", "원가", "예산 상한", "미공개 캠페인",
                    "미출시", "나만 보기", "대행사 단가")
...
def is_scheduled():
    """예약(무인) 실행이면 True. 루틴 환경이 MKT_COPILOT_SCHEDULED=1 을 넣어준다.
    무인 실행에서도 게시·발주·광고 집행은 절대 자동으로 나가지 않는다(승인 모드 적용)."""
    return os.environ.get("MKT_COPILOT_SCHEDULED", "").strip().lower() in ("1", "true", "yes", "on")


ENV_WEBHOOK = {"team": "MKT_COPILOT_SLACK_TEAM", "private": "MKT_COPILOT_SLACK_PRIVATE"}
...
# ============================================================
# 훅(hook) 공용 유틸 — 선제적 마케팅 운영자 동작(세션시작·요청·저장·종료)에서 쓴다.
# ============================================================
ACTIVITY_DIR = os.path.join(DATA_DIR, "_activity")
# 세션 종료 시 '게시했는데 성과 기록이 없는 콘텐츠' 목록을 남겨두는 자리 — 다음 세션의
# proactive가 짚는다. "측정 없는 게시 금지"를 기계적으로 보장하는 장치.
PENDING_UNMEASURED = os.path.join(DATA_DIR, "_pending_unmeasured")
...
def log_activity(kind, detail):
    """오늘 마케팅 활동 원장에 한 줄 남긴다(세션 종료 점검·주간 회고 등에 사용). 실패해도 조용히."""
    try:
