...
## 실행 유형: [A] 준비도 체크·설계·계산·판정 초안은 자동 / [P] 집행 개시·예산 증액은 승인 후 / **손실 차단(등록된 중단조건 발동 시 일시중지·입찰 하향)은 `approval_mode=auto`에서 자동 실행 후 즉시 보고** / [E] 월 예산 상한 초과·경쟁사 비교 광고·가격 조건 변경은 상신 — 무인 실행에서 집행 개시·증액은 자동 금지, 손실 차단은 허용(§13-6). 가드레일(fail-closed): 같은 룰 24시간 쿨다운·룰당 하루 5회 상한·하루 자동 변경 총액 상한, 현재 예산·오늘 사용량을 확인 못 하면 실행하지 말고 승인 대기로. 자세히 [[role]]
